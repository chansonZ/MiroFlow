...
class FallbackFinalAnswerGenerator(BaseIOProcessor):
    ...
    async def run_internal(self, ctx: AgentContext) -> AgentContext:
        ...
        logger.info(
            "FallbackFinalAnswerGenerator: final_boxed_answer=%r | "
            "llm_extracted_final_answer=%r | summary=%r | message_history_len=%d",
            final_boxed[:80] if final_boxed else "",
            llm_extracted[:80] if llm_extracted else "",
            summary_val[:80] if summary_val else "",
            history_len,
        )
        ...
        logger.info(
            "FallbackFinalAnswerGenerator: no valid answer found (final_boxed=%r) — running fallback",
            final_boxed[:80] if final_boxed else "",
        )
        ...
        if b_result:
            logger.info(
                "FallbackFinalAnswerGenerator: B-style fallback succeeded, result=%r",
                b_result[:120],
            )
            return AgentContext(
        ...
        if a_result:
            logger.info(
                "FallbackFinalAnswerGenerator: A-style fallback succeeded, result=%r",
                a_result[:120],
            )
            return AgentContext(
