...
An agent that executes all tool calls returned in a single LLM response
**concurrently** (via asyncio.gather), instead of sequentially.

Workflow per iteration:
  1. LLM emits N independent tool calls in one response
  2. All N calls are dispatched concurrently and awaited together
  3. All N results are returned to the LLM in the next user message

This removes the O(N) serial latency for independent operations such as
issuing multiple search queries or scraping multiple URLs at once.
"""
...
from miroflow.agents.iterative_agent_with_rollback import (
    MCP_TAGS,
    REFUSAL_KEYWORDS,
)
...
@register(ComponentType.AGENT, "ParallelToolCallingAgent")
class ParallelToolCallingAgent(BaseAgent):
    """
    Agent with parallel (concurrent) tool execution.

    Unlike IterativeAgentWithToolAndRollback, which executes tool calls one at a
    time, this agent fans out all tool calls that the LLM emits in a single
    response and runs them concurrently with asyncio.gather.

    The companion prompt (prompt_parallel_tool_calling_agent.yaml) instructs the
    LLM to output multiple independent tool calls per response when possible,
    which lets this agent collapse N serial round-trips into 1.
    """
    ...
    async def _execute_tool_calls_concurrent(
        self, tool_calls: list, max_tool_calls: int = 10
    ) -> tuple[list, bool]:
        """Execute tool calls concurrently using asyncio.gather.

        Returns:
            (results, exceeded) where results is a list of (call_id, result)
            tuples, and exceeded indicates whether the call list was capped.
        """
        ...
        async def _execute_single(tool_call: dict):
        ...
        return list(results), exceeded

    # ------------------------------------------------------------------
    # Rollback helpers (identical to parent)
    # ------------------------------------------------------------------
    ...
    async def run_internal(self, ctx: AgentContext) -> AgentContext:  # noqa: C901
        tracer = get_tracer()
        ...
        while max_turns == -1 or turn_count < max_turns:
            ...
            if len(tool_and_sub_agent_calls) == 0:
                ...
                # ── KEY DIFFERENCE: concurrent execution ──────────────
                (
                    tool_results,
                    tool_calls_exceeded,
                ) = await self._execute_tool_calls_concurrent(tool_calls)

                if self.verbose:
                    print(
                        f"[Turn {turn_count}] Concurrent tool results: "
                        f"{len(tool_results)} tool(s) finished"
                    )
                    for r in tool_results:
                        result_preview = (
                            str(r[1].get("result", ""))[:200]
