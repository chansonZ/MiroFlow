...
class TaskExecutor:
    ...
    async def _run_task(
        ...
        try:
            ...
            logger.info(
                "[task:%s] raw results — "
                "final_boxed_answer=%r | summary=%r | "
                "llm_extracted_final_answer=%r | exceed_max_turn_summary=%r",
                task_id,
                raw_final_answer[:120] if raw_final_answer else "",
                raw_summary[:120] if raw_summary else "",
                raw_llm_extracted[:120] if raw_llm_extracted else "",
                exceed_max_turn_summary[:120] if exceed_max_turn_summary else "",
            )
            ...
            if _is_placeholder(final_answer) and exceed_max_turn_summary:
                logger.info(
                    "[task:%s] final_answer is placeholder %r — replacing with exceed_max_turn_summary",
            ...
            if _is_placeholder(summary):
                if exceed_max_turn_summary:
                    logger.info(
                        "[task:%s] summary is placeholder %r — replacing with exceed_max_turn_summary",
                        task_id, summary,
                    )
                    summary = exceed_max_turn_summary
                elif final_answer:
                    logger.info(
                        "[task:%s] summary is placeholder — copying from final_answer",
                        task_id,
                    )
                    summary = final_answer

            logger.info(
                "[task:%s] final results — final_answer=%r | summary=%r",
                task_id,
                final_answer[:120] if final_answer else "",
