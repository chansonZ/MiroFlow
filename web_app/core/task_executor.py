...
class TaskExecutor:
    ...
    async def _run_task(
        ...
        try:
            ...
            # Known placeholder strings that mean "no real answer was produced".
            # SummaryGenerator emits the skip-placeholder when the task hit the turn
            # limit mid-run; FinalAnswerExtractor emits the other two when LLM
            # extraction failed.  Treat all of them as equivalent to empty so the
            # safety net below can replace them with something useful.
